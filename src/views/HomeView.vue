<template>
  <v-main>
    <v-container>
      ...
      <SearchBar @buscar="buscarPorTitulo" />
      <MovieFilter @filtrar="filtrarPorGenero" @limpiar="cargarPopulares" />

      <v-progress-linear v-if="cargando" indeterminate color="primary" class="mb-4" />

      <v-alert v-if="error" type="error" class="mb-4">{{ error }}</v-alert>

      <v-alert v-if="!cargando && !error && peliculas.length === 0" type="info" class="mb-4">
...
import { obtenerPopulares, buscarPeliculas, descubrirPorGenero } from '@/services/tmdb.js'
